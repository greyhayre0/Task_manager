...
async def update_profile(
    ...
    if password and len(password) < 8:
        query_string = urlencode(
            {"error": ["Пароль должен быть минимум 8 символов"]}, doseq=True
        )
        return RedirectResponse(url=f"/profile?{query_string}", status_code=302)
...
async def delete_account(db: Session = Depends(get_db), user=Depends(get_current_user)):
    user_service = UserService(db)
    if user.role == UserRole.ADMIN:
        if user_service.get_admin_count() <= 1:
            query_string = urlencode(
                {"error": ["Нельзя удалить единственного администратора"]}, doseq=True
            )
            return RedirectResponse(url=f"/profile?{query_string}", status_code=302)
    try:
...
async def delete_user(
    ...
    if user_id == current_user.id:
        query_string = urlencode(
            {"error": ["Нельзя удалить самого себя тут"]}, doseq=True
        )
        return RedirectResponse(url=f"/profile?{query_string}", status_code=302)
