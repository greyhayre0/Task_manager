...
async def join_team(
    code: str = Form(...), db: Session = Depends(get_db), user=Depends(get_current_user)
):
    ...
    try:
        ...
        query_string = urlencode(
            {"error": [f"Команда с кодом '{code}' не найдена"]}, doseq=True
        )
...
async def remove_user_team(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail="Ошибка при удалении пользователя из команды"
        )
    return RedirectResponse(url="/profile", status_code=302)
